...
function DocumentationSidebar() {
  return aside(
    {class: "sticky top-18 min-w-[200px] h-screen lg:flex hidden"},
    div(
      {class: "px-4 xl:pl-0 pl-4 h-full overflow-y-auto"},
      ...(router || [])?.map(({children, title}) =>
        div(
          {class: "pb-2"},
          Label({class: "font-semibold"}, escHtml`${title}`),
          div(
              ...
              Button(
                {
                  ...
                  class: "p-0 justify-start h-fit font-normal",
                  href: `/#/docs/${path}`,
                  variant: "link",
                },
                escHtml`${title}`
...
export function DocumentationLayout() {
  return div(
    {class: "w-full", "un-cloak": ""},
    div(
      {class: "flex max-w-7xl m-auto p-4"},
      DocumentationSidebar(),
      div(
        {class: "pb-16 max-w-3xl w-full"},
...
export function DocumentationCodearea({
  ...
  const codeAreaStyle =
    "min-h-auto border border-border rounded-lg bg-black overflow-hidden relative";
  const preAreaStyle =
    "text-sm text-white max-h-[300px] h-full w-full overflow-auto p-4 hide-scrollbar";
  const indexStyle =
    "bg-muted min-w-6 min-h-6 flex items-center justify-center rounded-full font-mono text-sm mt-0.5";
...
export function DocumentationTableOfContents({
  ...
  return aside(
    {class: "sticky top-18 right-0 h-screen xl:block hidden min-w-[250px]"},
    div(
